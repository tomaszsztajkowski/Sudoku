...
class Sudoku:
    def __init__(self, data = None):
        ...
        for i in range(81): self.board_sqr[sqr_indexes[str((i // 9) // 3) + str((i % 9) // 3)]].append(self.data[i])
        
    def is_valid(self):
        for arrangement in [self.board, self.board_t, self.board_sqr]:  # checks every needed arrangement of numbers on board
            for line in arrangement:
                check = {i: False for i in range(1, 10)}                # dictionary with bool representation for appearence of numbers
                for n in line:                                          # simple flag check
                    if n[0] == 0: continue
    ...
    def current_board(self):
        return [[num[0] for num in line] for  line in self.board]       # returs flat board

    counter = 0
    def solve_backtracking(self, index = 0, functions = []):
        if self.board[index//9][index%9][0] != 0:                       # handles prefilled numbers
            if index == 80: return True                                 # -||-
            return self.solve_backtracking(index + 1, functions=functions)                   # -||-
        ...
        while True:
            ...
            if self.solve_backtracking(index + 1, functions=functions) == True: return True  # recursion
